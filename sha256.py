...
# from github: https://github.com/TheAlgorithms/Python/blob/master/hashes/sha256.py
...
class SHA256:
    def __init__(self, data: bytes, state=None, count=0) -> None:

        self.data = data

        # Allow custom internal state for length extension attacks
        if state is None:
            self.hashes = [
                0x6A09E667,
                0xBB67AE85,
        ...
        self.preprocessed_data = self.preprocessing(
            self.data,
            previous_length=count
        )

        self.final_hash()

    @staticmethod
    def preprocessing(data: bytes, previous_length: int) -> bytes: # MODIFIED: to add previous_length
        total_length = previous_length + len(data)
        padding = b"\x80"
        while (total_length + len(padding) + 8) % 64 != 0:
            padding += b"\x00"
        big_endian_integer = struct.pack(">Q", total_length * 8)
